...
\section{对x(t)进行STFT}
使用matlab的spectrogram函数，先对输入信号进行离散采样，采样频率为$1000HZ > 100HZ$，故满足奈奎斯特抽样定理，共选择三组参数$\delta = 0.01$(见Fig\ref{delta001}，Fig\ref{delta0013d})，$\delta = 0.1$(见Fig\ref{delta01}，Fig\ref{delta013d})和$\delta = 1$(见Fig\ref{delta1}，Fig\ref{delta13d})，分别作图。:
\begin{figure}
    \centering
    \includegraphics[scale = 0.6]{delta001.jpg}
    \caption{STFT(x(t))图像($\delta = 0.01$)\label{delta001}}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[scale = 0.35]{delta0013d.jpg}
    \caption{STFT(x(t))3d图像($\delta = 0.01$)\label{delta0013d}}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[scale = 0.6]{delta01.jpg}
    \caption{STFT(x(t))图像($\delta = 0.1$)\label{delta01}}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[scale = 0.35]{delta013d.jpg}
    \caption{STFT(x(t))3d图像($\delta = 0.1$)\label{delta013d}}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[scale = 0.6]{delta1.jpg}
    \caption{STFT(x(t))图像($\delta = 1$)\label{delta1}}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[scale = 0.35]{delta13d.jpg}
    \caption{STFT(x(t))3d图像($\delta = 1$)\label{delta13d}}
\end{figure}
